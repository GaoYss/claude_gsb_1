...
| 树木档案 | `/trees` | 一树一档（树种、胸径、树高、树龄、保护级别、责任单位、经纬度精确点位并关联绿地）、按保护级别/行政区/绿地/生长势检索、复壮/支撑/防腐等养护措施逐次记录、绿地档案联动 |
...
│   │   │   ├── common.py        # 链式字段校验器
│   │   │   ├── filters.py       # 列表过滤条件
│   │   │   └── green_space.py / maintenance_task.py / maintenance_record.py / plant_replacement.py / tree.py
...
│   │   └── views/               # 视图：dashboard / green-space / task / record / replacement / tree
...
首次启动会自动建表；`SEED_DEMO_DATA=true` 时会写入一批演示数据（7 处绿地、15 条任务、22 条养护记录、8 条更换记录、7 株树木档案、20 条养护措施）。停止与清理：
...
| GET/POST | `/trees` | 树木档案列表（`keyword`/`district`/`green_space_id`/`protection_level`/`vigor`，返回按保护级别的汇总） / 树木建档 |
| GET | `/trees/districts` | 有树木建档的行政区及株数 |
| GET | `/trees/{id}` `/trees/{id}/profile` | 树木详情 / 树木档案（基础信息 + 养护统计 + 全部养护措施） |
| PUT/DELETE | `/trees/{id}` | 更新树木档案（编号不可改） / 删除（养护措施级联清除） |
| GET/POST | `/trees/{id}/maintenances` | 某株树下的养护措施列表 / 登记养护措施（复壮、支撑、防腐等） |
| PUT/DELETE | `/trees/{id}/maintenances/{careId}` | 更新 / 删除一条养护措施（跨树访问返回 404） |
| GET | `/statistics/dashboard` | 看板聚合数据（总览 + 分布 + 趋势 + 榜单 + 提醒 + 最近动态） |
| GET | `/statistics/overview` `/distributions` `/trends` `/ranking` `/reminders` | 看板分项接口 |

## 六、业务规则

1. **业务编号**：绿地 `GS-年份-序号`（如 `GS-2026-0001`），任务 `MT-YYYYMMDD-序号`，养护记录 `MR-YYYYMMDD-序号`，更换记录 `PR-YYYYMMDD-序号`，树木档案 `TR-年份-序号`（如 `TR-2026-0001`）；留空自动生成，唯一约束冲突时自动重试，编号创建后不可修改。
...
6. **删除保护**：删除绿地时若已存在任务/记录/更换/树木档案数据会返回 409 并给出数量明细，需 `force=true` 才级联删除；删除任务时养护记录默认保留（解除关联），避免养护履历丢失。
7. **字典单一来源**：所有枚举在 `backend/app/constants.py` 定义，前端通过 `/meta/enums` 获取并缓存，前后端不重复维护。
8. **一树一档**：每株树一条 `tree_profile`，登记树种、胸径、树高、冠幅、树龄、保护级别（名木/一至三级/暂无）、生长势、责任单位与责任人；位置可填文字点位，也可填经纬度精确定位（二者需成对出现），并强制关联到所在绿地。复壮、支撑、防腐、树洞修补、病虫害防治等养护措施按实施日期逐次记录在该档案下（`tree_maintenance`），删除树木时措施级联清除；按保护级别、行政区（经绿地关联）、绿地、生长势与关键字均可检索。
...
| `tree_profile` | 树木档案（一树一档） | `tree_no`(唯一)、`green_space_id`、`tree_species`、`scientific_name`、`dbh_cm`、`height_m`、`crown_width_m`、`age_years`、`protection_level`、`vigor`、`responsible_unit`、`location_desc`、`longitude`/`latitude`、`register_date` |
| `tree_maintenance` | 树木养护措施 | `tree_id`(级联)、`care_type`、`care_date`、`content`、`operator`、`result`、`cost` |

绿地删除时任务/记录/更换/树木档案级联清理；树木档案删除时其养护措施级联清理；任务与养护记录之间、养护记录与更换记录之间为可空外键（`SET NULL`），保证养护履历可独立留存。

## 八、测试

```bash
cd backend
python -m pytest              # 72 个用例：接口、校验、跨模块规则、端到端流程
```

覆盖重点：绿地编号生成与唯一性、枚举与字段校验、列表过滤/排序/分页、任务状态自动流转与手动流转限制、记录删除后的状态回退、更换金额核算、删除保护与强制删除、统计聚合口径一致性、树木建档与经纬度/树龄校验、按保护级别与行政区检索、养护措施逐次登记与跨树隔离、树木与绿地的级联删除、演示数据自洽性。
