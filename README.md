...
| 绿地台账 | `/green-spaces` | 绿地建档（编号自动生成）、按行政区/类型/等级/状态/关键字检索、档案详情（概览 + 近期任务/记录/更换 + 树木档案 + 更换原因汇总）、删除保护 |
...
| 树木档案 | `/trees` | 一树一档（树种、胸径、树高、树龄、保护级别、责任单位、经纬度点位）、复壮/支撑/防腐等措施逐次登记、按保护级别/行政区/绿地检索、档案与措施统计、删除保护 |
...
│   │   │   ├── common.py        # 链式字段校验器
│   │   │   ├── filters.py       # 列表过滤条件
│   │   │   └── green_space.py / maintenance_task.py / maintenance_record.py / plant_replacement.py / tree.py / tree_maintenance.py
...
│   │   │   ├── tree_service.py / tree_maintenance_service.py
...
首次启动会自动建表；`SEED_DEMO_DATA=true` 时会写入一批演示数据（7 处绿地、18 条任务、24 条养护记录、8 条更换记录、8 份树木档案、13 次树木养护措施）。停止与清理：
...
| GET/POST | `/trees` | 树木档案列表（`keyword`/`protection_level`/`growth_vigor`/`district`/`green_space_id`，返回汇总） / 树木建档（编号可留空自动生成） |
| GET/PUT/DELETE | `/trees/{id}` | 档案详情（含措施统计与逐次措施记录） / 更新（编号不可改） / 删除（有措施时需 `force`） |
| GET | `/trees/summary` | 树木汇总（总数、保护级别分布、覆盖行政区、措施次数） |
| GET/POST | `/tree-maintenances` | 措施列表（`tree_id`/`measure_type`/日期区间） / 登记措施（复壮、支撑、防腐等） |
| GET/PUT/DELETE | `/tree-maintenances/{id}` | 措施详情 / 更新 / 删除 |
| GET | `/statistics/dashboard` | 看板聚合数据（总览 + 分布 + 趋势 + 榜单 + 提醒 + 最近动态） |
| GET | `/statistics/overview` `/distributions` `/trends` `/ranking` `/reminders` | 看板分项接口 |

## 六、业务规则

1. **业务编号**：绿地 `GS-年份-序号`（如 `GS-2026-0001`），任务 `MT-YYYYMMDD-序号`，养护记录 `MR-YYYYMMDD-序号`，更换记录 `PR-YYYYMMDD-序号`，树木档案 `TR-年份-序号`，树木养护措施 `TM-YYYYMMDD-序号`；留空自动生成，唯一约束冲突时自动重试，编号创建后不可修改。
...
4. **日期约束**：养护日期、更换日期不得早于绿地建成日期；树木养护措施的实施日期同样不得早于所在绿地建成日期。
5. **金额核算**：更换金额 = 数量 × 单价，由后端统一计算；未填单价时金额留空，前端提示补录。
6. **删除保护**：删除绿地时若已存在任务/记录/更换/树木档案数据会返回 409 并给出数量明细，需 `force=true` 才级联删除；删除任务时养护记录默认保留（解除关联），避免养护履历丢失；删除树木档案时若已登记养护措施需 `force=true`，措施记录随档案一并清除。
...
| `tree` | 树木档案 | `code`(唯一)、`green_space_id`、`species`、`dbh_cm`、`height_m`、`age_years`、`protection_level`、`growth_vigor`、`responsible_unit`、`longitude`、`latitude`、`location_desc` |
| `tree_maintenance` | 树木养护措施 | `record_no`(唯一)、`tree_id`、`measure_type`、`measure_date`、`content`、`operator` |

绿地删除时任务/记录/更换/树木档案级联清理；任务与养护记录之间、养护记录与更换记录之间为可空外键（`SET NULL`），保证养护履历可独立留存；树木养护措施随树木档案级联清理。
...
覆盖重点：绿地编号生成与唯一性、枚举与字段校验、列表过滤/排序/分页、任务状态自动流转与手动流转限制、记录删除后的状态回退、更换金额核算、树木建档与经纬度校验、按保护级别/行政区检索树木、措施逐次登记与日期约束、删除保护与强制删除、统计聚合口径一致性、演示数据自洽性。
