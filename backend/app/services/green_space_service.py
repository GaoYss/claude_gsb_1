...
from ..models import GreenSpace, MaintenanceRecord, MaintenanceTask, PlantReplacement, Tree
...
class GreenSpaceService(BaseService):
    ...
    def delete(cls, obj_id, force=False):
        ...
        counts = {
            ...
            .scalar()
            or 0,
            "tree": db.session.query(func.count(Tree.id))
            .filter(Tree.green_space_id == space.id)
            .scalar()
            or 0,
        }
        if sum(counts.values()) and not force:
            raise ConflictError(
                ...
                "绿植更换记录 {plant_replacement} 条、树木档案 {tree} 份，"
